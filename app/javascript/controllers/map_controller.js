  ...
  #getRoute(fetchQueryString) {
    fetch(fetchQueryString)
      ...
      .then((data) => {
        ...
        let totalTimeMins =
          (Math.round(data.routes[0].duration / 60) + 45 * counter) % 60;
        let totalTimeHours = Math.round(
          (Math.round(data.routes[0].duration / 60) + 45 * counter) / 60
        );
        // console.log("------------------")
        // console.log(totalTimeHours + ":" + totalTimeMins)
        // console.log("------------------")
        this.element.parentElement.parentElement.parentElement.children[0].querySelector(
          ".estimated-time"
        ).innerHTML = `Est BarHop duration: ${totalTimeHours} hours ${totalTimeMins} mins`;
      });
  }
}
